...
const Header: React.FC<HeaderProps> = ({ onOpenSidebar, isBottomMode }) => {
  return (
    <header className={`sticky top-0 z-20 flex items-center px-8 py-5 bg-[#F8F9FB]/80 backdrop-blur-xl ${isBottomMode ? 'justify-center gap-6' : 'justify-between'}`}>
      <div className={`flex items-center gap-8 ${isBottomMode ? 'w-auto' : 'flex-1'}`}>
        {/* 收缩按钮 - 极简线条 */}
        {!isBottomMode && (
          <button 
            onClick={onOpenSidebar}
            className="w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-[#EEEEEE] text-[#1A1A1A] hover:border-[#33a3dc]/40 transition-all active:scale-95 shadow-sm"
          >
        ...
        {/* 呼吸感搜索栏 */}
        <div className={`relative group ${isBottomMode ? 'w-64 md:w-80' : 'flex-1 max-w-lg'}`}>
